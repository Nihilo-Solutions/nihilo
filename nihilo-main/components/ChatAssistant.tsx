...
const ChatAssistant: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Try calling a backend endpoint first (if you add one). If unavailable, fall back to a local stub.
      ...
      try {
        ...
        if (res.ok) {
          ...
          responseText = data?.text || data?.response || null;
        }
      } catch (err) {
        // ignore — we'll use a local fallback below
      }

      if (!responseText) {
        // Local offline stub so the UI still works without a server-side Gemini integration.
        responseText = `Offline Assistant: Received "${userMessage}" — Gemini integration not configured.`;
      }
