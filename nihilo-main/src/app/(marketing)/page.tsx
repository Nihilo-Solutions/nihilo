      ...
      <section className="pt-12 pb-16 sm:pt-16 sm:pb-20">
        <div className="mx-auto grid w-[min(70rem,calc(100%-2.5rem))] items-center gap-10 lg:grid-cols-[1fr_22rem]">
          ...
          <aside className="rounded-lg border border-border bg-bg-elev p-6 shadow-[var(--shadow-card)]">
            <p className="text-xs tracking-[0.16em] text-muted uppercase">Still done by hand</p>
            <ol className="mt-4 space-y-3 border-l border-border pl-4 text-sm">
              <li>Gather the numbers</li>
              <li>Fill in the template</li>
              <li>Write the summary</li>
              <li>Send it out</li>
            </ol>
            <p className="mt-4 text-sm text-muted">One person. Same steps every time. Same chance something is missing.</p>
      ...
      <section className="border-y border-border bg-bg-alt py-20">
        <div className="mx-auto w-[min(70rem,calc(100%-2.5rem))]">
          ...
          <p className="mt-3.5 max-w-xl text-lg text-muted">
            Industry does not matter as much as a process that repeats. If someone does it on a
            cycle, it is a candidate.
          </p>
          <div className="mt-10 grid gap-4.5 md:grid-cols-3">
            <WorkCard
              num="01"
              title="Operational reports"
              body="Gather the inputs, apply the same rules, and render the pack into your template on a schedule. Client reports, status packs, exception lists. Nobody assembles it."
              href="/use-cases/reports"
            />
            <WorkCard
              num="02"
              title="Data prep and cleanup"
              body="The checking, correcting and reformatting somebody does before data can go into a system. Rules instead of habits, and only the genuinely broken records reach a person."
              href="/use-cases/data-prep"
            />
            <WorkCard
              num="03"
              title="Client and record lookup"
              body="Find the right customer, job, or account without searching several systems by hand. One question. The right record."
